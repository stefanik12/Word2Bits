...
void *TrainModelThread(void *id) {
  ...
  // Added variable learning rate
  printf("Before alphas def");
  float alphas [layer1_size];
  printf("After alphas def");
  ...
  while (1) {
	...
	// Added variable learning rate
	for (int a_i = 0; a_i < layer1_size; a_i++) {
	  alphas[a_i] = alpha;
	}
	
	for (c = 0; c < layer1_size; c++) {
	  context_avge[c] += alpha * g * quantize(v[c + l2], local_bitlevel);
	}
	for (c = 0; c < layer1_size; c++) {
	  v[c + l2] += g * context_avg[c] - 2*alphas[c]*reg*v[c + l2];
	}
	...
	for (int i = 0; i < layer1_size; i++) {
	    sum_loss = sum_loss + v[i];
	}

	real local_loss = log(sigmoid(dot_product)) * pow(sum_loss, 2);
	...
	/////////////////////
	  ...
	  if (last_word == -1) continue;
	  for (c = 0; c < layer1_size; c++) {
	    u[c + last_word * layer1_size] += context_avge[c] - 2*alphas[c]*reg*u[c+last_word*layer1_size];
	  }
	}
    }
    sentence_position++;
